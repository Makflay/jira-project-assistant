...
import { MutationResult } from "./types/defs";
...
resolver.define(
  "getIssuesByProject",
  async ({ payload }): Promise<JiraIssue[]> => {
...
resolver.define(
  ...
  async ({ payload }): Promise<JiraUser[]> => {
    const { projectKey } = payload as { projectKey: string };
...
resolver.define("assignIssue", async ({ payload }): Promise<MutationResult> => {
...
resolver.define(
  "updateIssuePriority",
  async ({ payload }): Promise<MutationResult> => {
